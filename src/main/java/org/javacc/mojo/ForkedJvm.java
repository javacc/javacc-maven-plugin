...
/**
 * Runs the <code>main()</code> method of some tool in a forked JVM.<br>
 * Used when the tool calls System.exit() (to not kill the entire Maven process).
 */
class ForkedJvm {
   ...
   *
   * @param directory The working directory for the forked JVM, may be <code>null</code> to inherit
   *     the working directory of the current JVM.
   */
  ...
  private String getClassPath() {
    return StringUtils.join(classPathEntries.iterator(), File.pathSeparator);
  }
  ...
  public void addClassPathEntry(final File path) {
    if (path != null) {
      classPathEntries.add(path.getAbsolutePath());
    }
  }
   ...
   *
   * @param type The class on which to invoke the <code>main()</code> method, may be <code>null
   *     </code>.
   */
  public void setMainClass(final Class<?> type) {
    mainClass = (type != null) ? type.getName() : null;
  ...
  private String[] getArguments() {
    return (String[]) cmdLineArgs.toArray(new String[cmdLineArgs.size()]);
  }
  ...
  public void addArgument(final File argument) {
    if (argument != null) {
      cmdLineArgs.add(argument.getAbsolutePath());
    }
  }
  ...
  private Commandline createCommandLine() {
    ...
    if (mainClass != null && mainClass.length() > 0) {
      cli.addArguments(new String[] {mainClass});
    }
   ...
   * @throws CommandLineException If the JVM could not be forked.
   */
  public int run() throws CommandLineException {
    return CommandLineUtils.executeCommandLine(createCommandLine(), systemOut, systemErr);
  }
