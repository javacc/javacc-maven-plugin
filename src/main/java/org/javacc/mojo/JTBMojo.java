...
/**
 * Parses a JTB file and transforms it into source files for an AST and a JavaCC grammar file which
 * automatically builds the AST.
 *
 * @goal jtb
...
public class JTBMojo extends AbstractPreprocessorMojo {
   ...
   * The default value is <code>${project.build.directory}/generated-sources/jtb-timestamp</code>. A
   * plugin parameter.
   *
   ...
   * processing. By default, the patterns <code>**&#47;*.jtb</code> and <code>**&#47;*.JTB</code>
  ...
  protected void processGrammar(final GrammarInfo grammarInfo)
    ...
    final File nodeDirectory =
        new File(getOutputDirectory(), nodePackage.replace('.', File.separatorChar));

    final String visitorPackage = grammarInfo.resolvePackageName(getVisitorPackageName());
    final File visitorDirectory =
        new File(getOutputDirectory(), visitorPackage.replace('.', File.separatorChar));
   ...
   * current values of the corresponding mojo parameters.<br>
